...
export const runAllCRUDTests = async () => {
  ...
  allResults.noticias = await runCRUDTests(
    'noticias',
    {
      ...
      autor: 'Testador',
      imagem: null // Será adicionada via teste de imagem
    },
    {
  ...
  allResults.galeria = await runCRUDTests(
    'galeria',
    {
      ...
      categoria: 'teste',
      imagem: null // Obrigatório para galeria
    },
    {
...
/**
 * Testa funcionalidades específicas do Realtime Database
 * @returns {Promise<object>} Resultados dos testes específicos
 */
export const runRealtimeDatabaseTests = async () => {
  const results = {};
  
  try {
    // Teste de leitura de coleção completa
    console.log('Testando leitura de coleção completa...');
    const noticiasResult = await readCollection('noticias', { limit: 5 });
    results.collectionRead = {
      success: noticiasResult.success,
      count: noticiasResult.count || 0,
      message: noticiasResult.message
    };
    
    // Teste de funções específicas
    console.log('Testando funções específicas...');
    
    // Criar notícia de teste usando função específica
    const noticiaTest = await createNoticia({
      titulo: 'Teste Função Específica',
      categoria: 'teste',
      resumo: 'Teste usando função específica de notícia',
      conteudo: 'Conteúdo de teste',
      autor: 'Sistema de Teste'
    });
    
    results.specificFunction = {
      success: noticiaTest.success,
      message: noticiaTest.message,
      id: noticiaTest.id
    };
    
    // Limpeza: excluir notícia de teste se foi criada
    if (noticiaTest.success) {
      await deleteNoticia(noticiaTest.id);
    }
...
export const runAllTests = async () => {
  ...
  try {
    ...
    allResults.crud = await runAllCRUDTests();
    
    // Testes específicos do Realtime Database
    console.log('Executando testes específicos do Realtime Database...');
    allResults.realtime = await runRealtimeDatabaseTests();
    
    // Teste de processamento de imagem (simulado)
    console.log('Testando processamento de imagem...');
    try {
      ...
      canvas.toBlob(async (blob) => {
        if (blob) {
          ...
          allResults.imageProcessing = await testImageProcessing(file, true);
        }
      }, 'image/png');
    } catch (error) {
      allResults.imageProcessing = {
        success: false,
        message: 'Teste de imagem não disponível no ambiente atual'
      };
    }
  ...
  console.log('=== Resultados finais de todos os testes ===', allResults);
  return allResults;
};
